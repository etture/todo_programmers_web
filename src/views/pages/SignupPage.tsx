...
class SignupPage extends Component<ISignupPageProps, ISignupPageState> {
	...
	handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		axiosInstance.post('/auth/signup', { nickname: this.state.nickname, password: this.state.password })
			.then((result) => {
				log('signup result: ', result.data.success, result.data.userId);
			...
			<div className="container mt-5 col-6">
				<div className="card">
					<div className="card-header">
						<h4 className="d-flex align-content-center">TODO 회원가입</h4>
					</div>
					<div className="card-body">
						<form onSubmit={(e: React.FormEvent<HTMLFormElement>) => this.handleSubmit(e)}>
							...
							<Link className="ml-3" to='/auth/signin'>뒤로 가기</Link>
						</form>
					</div>
				</div>
			</div>
